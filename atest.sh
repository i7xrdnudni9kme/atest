...
E[0]="Language:\n 1. English (default) \n 2. 简体中文"
C[0]="${E[0]}"
E[1]="1. Add the option to use Warp IPv4 for returning to China; 2. Add a number of quality cdn's that are collected online. 3. Use Warp IPv6 to visit chatGPT."
C[1]="1. 增加使用 Warp IPv4 回国选项; 2. 增加线上收录的若干优质 cdn; 3. 使用 Warp IPv6 访问 chatGPT"
...
E[49]="Do you use warp IPv4 to access mainland websites?\n Pros: Increase security by using Cloudflare on 104.28.x.x to access domestic websites.\n Cons: Slows down access."
C[49]="是否使用 warp IPv4 访问大陆网站功能?\n 优点: 使用 104.28.x.x 的 Cloudflare 访问国内网站，增加安全性\n 缺点: 减慢访问速度"
E[50]="Use warp IPv4 to access mainland websites"
C[50]="warp IPv4 回国"
E[51]="Install Sing-box multi-protocol scripts."
C[51]="安装 Sing-box 协议全家桶脚本"
...
EOF
  ...
  # 禁止回国开启时，修改路由规则文件
  [[ "$RETURN" =~ 'y' ]] && sed -i "/outboundTag/s/direct/warp-IPv4/g" $WORK_DIR/outbound.json
  ...
  hint " $(text 41) \n" && reading " $(text 24) " CHANGE_TO
    case "$CHANGE_TO" in
      1 ) systemctl disable --now argo
          ...
          sed -i "s@ExecStart.*@ExecStart=$WORK_DIR/cloudflared tunnel --edge-ip-version auto --protocol http2 --no-autoupdate --url http://localhost:2082@g" /etc/systemd/system/argo.service
...
switch_return() {
  if grep -q 'outboundTag.*direct' $WORK_DIR/outbound.json; then
    sed -i "/outboundTag/s/direct/warp-IPv4/g" $WORK_DIR/outbound.json
  else
    sed -i "/outboundTag/s/warp-IPv4/direct/g" $WORK_DIR/outbound.json
  fi
